...
def get_title(vid_id):
    ...
    @type vid_id: str
    """
    video_info = urllib.request.urlopen('http://youtube.com/get_video_info?video_id=' + vid_id)
    video_info = video_info.read()

    response=urllib.parse.parse_qs(video_info)[b'player_response']

    dict_str = response[0].decode("UTF-8")
    video_data =json.loads(dict_str)
    for item in video_data:
        print(item)
        print(video_data[item])
    if video_data['playabilityStatus']['status'] == 'fail':
        print("WARNING: Couldn't get video title. This probably means you specified an invalid URL.")
        return None
    else:
        return video_data['videoDetails']['title']
...
def get_transcript():
    ...
    try:
        transcript = urllib.request.urlopen('http://video.google.com/timedtext?lang=en&v=' + vidinfo.id)
        transcript_xml = transcript.read()
        transcript_xml=transcript_xml.decode('UTF-8')
