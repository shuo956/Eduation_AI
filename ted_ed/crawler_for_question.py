...
def collect_category_relation(path):    
    ...
    # Log in
    driver.find_element_by_xpath("//input[@data-testid='lookup__username__1']").send_keys('shuo956@outlook.com')
    ...
    for i in range(1, total_pages+1):
        ...
        for tuple in video_tuples:
            ...
            try:
                ...
                comment = re.compile('<div class="question scrollable text-ultralight">(?:.|\n).*')

                find = re.findall(comment, ele)
                find = [item.split('\n')[1].strip() for item in find]
                question=find
                temp_dic={}
                temp_dic['title']=video_title_length
                temp_dic['multi']=question
                temp_list_for_questions.append(temp_dic)
           ...
           #             driver.get(video_link + "/review_open#question-" + str(j + 2))
           #
           #             quiz_text = driver.find_element_by_xpath("(//div[@class='question scrollable text-ultralight'])").text
           #             print(j)
           #             print(quiz_text)
           ...
           #         collected_videos.add(video_title_length)
                ...
                pass
    for item in temp_list_for_questions:
        f.write(str(item)+'\n')

def main():
    data_path = '../../data/teded/teded_crawled_data_new/'
